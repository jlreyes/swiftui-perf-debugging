---
name: swiftui-scroll-animation
description: Diagnoses scroll and animation jank in SwiftUI macOS/iOS apps using a frame-first workflow. Covers interaction windowing, severity buckets, cascade analysis, per-frame attribution, root cause clustering, and a final report template. Requires the claude-instruments skill for trace export and schema reference.
---

# SwiftUI Scroll & Animation Jank Diagnosis

> **Prerequisite**: This skill requires [claude-instruments](https://github.com/jlreyes/claude-instruments) for recording traces, exporting to DuckDB, and the schema reference. Install it first:
> ```bash
> claude install-skill jlreyes/claude-instruments
> ```

Use this skill when investigating **scroll lag**, **animation stutter**, and **frame drops** in SwiftUI apps.
...
- [ ] Collect: record trace under realistic conditions; export to DuckDB (see [claude-instruments](https://github.com/jlreyes/claude-instruments))
...
Prefer Release build behavior. Use the recording workflow from [claude-instruments](https://github.com/jlreyes/claude-instruments):
...
Use the export script from claude-instruments (run it directly, not with `python3`):
...
Use the `prepare_analysis.py` script from claude-instruments to create the `frames` view:
...
- Full schemas and extra SQL patterns: see SCHEMAS.md in [claude-instruments](https://github.com/jlreyes/claude-instruments)
- General Instruments → DuckDB workflow: see SKILL.md in [claude-instruments](https://github.com/jlreyes/claude-instruments)
- Export trace to DuckDB: `scripts/export_to_duckdb.py` (from claude-instruments)
- **Primary analysis tool**: `scripts/prepare_analysis.py` (from claude-instruments)
